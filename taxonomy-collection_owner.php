<?php
/*
Description: Collection Owner taxonomy archive.
*/
                ...
                <table class="table table-striped table-sorter tech-list">
                  ...
                  <?php endif; ?>
					...
					<div id="grid_view" class="directory row view view-active">
					 ...
					 <?php if (have_posts()) : while (have_posts()) : the_post(); ?>
							<?php $i++; ?>
              <?php $location_terms = get_the_term_list( $post->ID, 'collection_location', '', ' ', '' );
                    $keyword_terms = get_the_term_list( $post->ID, 'collection_keyword', '', ' ', '' );
                    $location_terms = strip_tags( $location_terms );
                    $keyword_terms = strip_tags( $keyword_terms );
               ?>
							<div class="col-xs-6 col-md-4 col-lg-3 <?php echo $location_terms.' '.$keyword_terms ?>">
				    		<div class="thumbnail">
                  <figure><a href="<?php echo get_permalink(); ?>"><?php the_post_thumbnail('staff-thumbnail', array('class' => 'staff-thumbnail')); ?></a></figure>
                    ...
                    <?php if(get_the_term_list( $post->ID, 'collection_location', true) ||
                      ...
                      get_post_meta($post->ID, 'availability', true)
      								...
      								<ul>
                      <?php if(get_the_term_list( $post->ID, 'collection_location', true)): ?>
                        <li><i class="fa fa-info-circle" data-toggle="tooltip" data-placement="right" title="Tech Type"></i><?php echo get_the_term_list( $post->ID, 'collection_location', '', ', ', '' ); ?></li>
                      <?php endif; ?>
